...
pub fn dispatch_frontend_event(state: &mut State, event: Event) {
    match event.payload {
        Payload::OutOfMapBounds(_) => {
            common_core::utils::log::print_string("out of map bounds".to_owned())
        }
        ...
        Payload::SelectBuilding(payload) => {
            select_building(state, payload.renderable, payload.frame)
        }
